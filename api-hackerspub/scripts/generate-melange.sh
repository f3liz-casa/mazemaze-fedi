#!/usr/bin/env bash
# Generate the Melange-backed typed layer for the hackers.pub GraphQL API with
# mazemaze-generator:
...
# The GraphQL operations live in graphql/operations/ in this package.
set -euo pipefail

HERE="$(cd "$(dirname "$0")/.." && pwd)"
GENERATOR="${GENERATOR:-$HERE/../generator}"
OPERATIONS="${OPERATIONS:-$HERE/graphql/operations}"
...
(
  cd "$GENERATOR"
  dune build ./gen/driver.exe
  node graphql-probe/probe.mjs graphql-probe/schema.graphql "$OPERATIONS" > "$TMP/normalized.json"
  dune exec ./gen/driver.exe -- "$TMP" --graphql "$TMP/normalized.json"
)
